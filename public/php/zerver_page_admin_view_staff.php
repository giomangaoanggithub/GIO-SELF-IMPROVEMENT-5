...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM `admin_teacher_conn` INNER JOIN `user_accounts` ON admin_teacher_conn.teacher_email = user_accounts.email WHERE a_t_c_id = '$view_staff' AND admin_email = '$email'");
    $stmt->execute();
